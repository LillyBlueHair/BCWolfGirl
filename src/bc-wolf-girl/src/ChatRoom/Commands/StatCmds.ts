...
export const StatCmds: CommandTemplate[] = [
    {
        ...
        run(player, sender, content) {
            ...
            const arousal_report = GetWolfGirlName(player) + "的高潮统计如下：\n" +
                `  高潮次数：${orgasm}\n` +
                `  忍耐次数：${resist}\n` +
                `  毁灭次数：${ruined}\n` +
    ...
    },
    {
        ...
        run(player, sender, content) {
            ...
            const task_report = GetWolfGirlName(player) + "的训练统计如下：\n" +
        ...
        run(player, sender, content) {
            TimedWorker.global.push({
                description: "ScanBody", works: [
                    ...
                    new MessageWork({ mode: "action", msg: `{player_wg}的项圈上涌出了数股金属银色的小小水流，正沿着{player_wg}的身体缓缓流向身体各处，短暂的覆盖住了{player_wg}的身体，如同银色的乳胶衣一般覆盖了全身，随着一阵刺痒过后，这件奇特的乳胶衣又化回了银色的水流，原路返回项圈之中` }),
                    new DelayWork(5000),
